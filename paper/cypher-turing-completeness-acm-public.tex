...
MATCH REPEATABLE ELEMENTS
  p = (init:Init)
    (()-[t:INC|JZDEC_ZERO|JZDEC_POS]->())
      {1, 9223372036854775807}
    (h:Halt)
WHERE allReduce(
  m = {A: 0, B: 0}, t |
  CASE
    WHEN t:INC AND t.c = 'A'
      THEN {A: m.A+1, B: m.B}
    WHEN t:INC AND t.c = 'B'
      THEN {A: m.A, B: m.B+1}
    WHEN t:JZDEC_POS AND t.c = 'A'
      THEN {A: m.A-1, B: m.B}
    WHEN t:JZDEC_POS AND t.c = 'B'
      THEN {A: m.A, B: m.B-1}
    ELSE m
  END,
  CASE
    WHEN t:JZDEC_ZERO AND t.c = 'A'
      THEN m.A = 0
    WHEN t:JZDEC_ZERO AND t.c = 'B'
      THEN m.B = 0
    WHEN t:JZDEC_POS AND t.c = 'A'
      THEN m.A > 0
    WHEN t:JZDEC_POS AND t.c = 'B'
