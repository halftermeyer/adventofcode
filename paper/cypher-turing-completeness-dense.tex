...
\begin{tikzpicture}[
  >=Stealth,
  node distance=1.2cm,
  box/.style={rectangle, rounded corners=2pt, draw=neo-blue!60,
    fill=neo-marigold!12, thick, minimum height=0.7cm,
    text width=2.2cm, align=center, font=\tiny},
  decision/.style={diamond, draw=neo-blue!60, fill=neo-marigold!12,
    thick, minimum height=0.7cm, aspect=2, align=center, font=\tiny},
  action/.style={rectangle, rounded corners=2pt, draw=neo-teal,
    fill=neo-teal!10, thick, minimum height=0.6cm,
    text width=1.8cm, align=center, font=\tiny},
  every edge/.style={draw, thick, ->, >=Stealth},
  edgelabel/.style={font=\tiny, inner sep=1pt},
]
  \node[box] (read) {Read state\\{\ttfamily machine.state}};
  \node[box, right=1cm of read] (fetch) {Fetch instr\\{\ttfamily program[state]}};
  \node[decision, right=1cm of fetch] (type) {Instr\\type?};
  \node[action, above right=0.5cm and 1cm of type] (inc) {Increment\\Update state};
  \node[decision, right=1cm of type] (zero) {Counter\\$= 0$?};
  \node[action, below right=0.5cm and 1cm of type] (halt) {state $= -1$};
  \node[action, above right=0.4cm and 0.8cm of zero] (gz) {Go to $q_z$};
  \node[action, below right=0.4cm and 0.8cm of zero] (gp) {Dec, go $q_p$};
  ...
  \draw[->] (zero) -- node[edgelabel, above left] {Y} (gz);
  \draw[->] (zero) -- node[edgelabel, below left] {N} (gp);
\end{tikzpicture}%
}\\[1pt]
{\scriptsize\color{neo-muted}Fig.\,3 --- One \texttt{reduce()} iteration: read state, fetch, branch, update.}
\end{center}
